document.addEventListener("DOMContentLoaded", function(){
    ...
    var startX, currentX, isDragging = false;
    var threshold = 50; // 슬라이드 변경을 위한 최소 이동 거리
    ...
    // 터치 이벤트 핸들러
    $('#carousel-generic').on('touchstart', function(event) {
        startX = event.touches[0].pageX;
        isDragging = true;
    });

    $('#carousel-generic').on('touchmove', function(event) {
        if (!isDragging) return;
        currentX = event.touches[0].pageX;
    });

    $('#carousel-generic').on('touchend', function(event) {
        if (!isDragging) return;
        isDragging = false;
    ...
    // 마우스 이벤트 핸들러
    $('#carousel-generic').on('mousedown', function(event) {
        startX = event.pageX;
        isDragging = true;
        event.preventDefault(); // 드래그 중 선택 텍스트 방지
    });
    ...
    $(".js-modal-vimeo-showcase").modalVideo({
      channel: 'vimeo-showcase',
      vimeo: {
        api: false,
        autopause: false,
        autoplay: true,
...
//          format: 'H:mm',    // use this format if you want the 24hours timepicker
	...
	showNotification: function(from, align, msg, color_type){
