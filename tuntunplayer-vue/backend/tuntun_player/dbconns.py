...
class ConnectionPool:
    ...
    def __init__(self, connection_string, min_connections=2, max_connections=10):
...
def execute_without_return(proc_name, params):
    """저장 프로시저 실행 (반환값 없음)"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                if params:
                    ...
                    cursor.execute(proc_name)
                conn.commit()
    except Exception as e:
        current_app.logger.error(f"프로시저 실행 중 오류 (반환값 없음): {str(e)}")
        raise

def execute_return(proc_name, params):
    """저장 프로시저 실행 (단일 결과 반환)"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if cursor.description:
                    ...
                    # 결과 집합이 없는 경우 None 반환
                    return None
    except Exception as e:
        current_app.logger.error(f"프로시저 실행 중 오류 (단일 결과): {str(e)}")
        raise
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if cursor.description:
                    ...
                    # 결과 집합이 없는 경우 빈 리스트 반환
                    return []
    except Exception as e:
        current_app.logger.error(f"프로시저 실행 중 오류 (결과 목록): {str(e)}")
        raise
